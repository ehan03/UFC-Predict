...
def get_info_from_fighter(fighter_url):
    ...
    for row in rows:
        entry = []
        for i, atag in enumerate(
            row.findAll("a", {"class": "b-link b-link_style_black"}, href=True)
        ):
            if i == 2:
                entry.append(atag["href"])
        for i, ptag in enumerate(
            row.findAll("p", {"class": "b-fight-details__table-text"})
        ):
            if i == 11 or i == 12:
    ...
    info = []
    if uid and name and record:
        info.append(uid)
        info.append(name[0].text.strip())
        info.append(record[0].text.replace("Record: ", "").strip())
    else:
        print(soup.prettify())
...
def get_event_urls_and_fighter_stats(fighter_urls):
    ...
    df1 = pd.DataFrame(events, columns=["url", "event", "date"]).drop_duplicates(
        keep="first"
    )
...
def main():
    ...
    print(
        f"Found statistics for {bout_stats.shape[0]} bouts, now cleaning data...",
        flush=True,
    )
